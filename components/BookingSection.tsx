  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ ...formData, name: "", email: "", phone: "", services: [] });
      } else {
    ...
    <section id="booking" className="py-20 bg-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6"> Réservez nos services </h2>
          <p className="text-lg text-gray-700"> Remplissez le formulaire ci-dessous pour réserver nos services. </p>
        </div>
        <Card>
          <CardHeader>
            <CardTitle>Formulaire de réservation de service</CardTitle>
            <CardDescription> Veuillez fournir vos coordonnées et sélectionner les services dont vous avez besoin. </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {services.map((service) => (
                    <div key={service} className="flex items-center space-x-2">
                      <Checkbox id={service} checked={formData.services.includes(service)} onCheckedChange={(checked) => handleServiceChange(service, checked as boolean)} />
                ...
                <div className="space-y-4 border-t pt-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-2">
                      {cleaningTypes.map((type) => (
                        <div key={type} className="flex items-center space-x-2">
                          <Checkbox id={type} checked={formData.cleaningTypes.includes(type)} onCheckedChange={(checked) => handleArrayFieldChange('cleaningTypes', type, checked as boolean)} />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input id="propertySize" type="number" value={formData.propertySize} onChange={(e) => setFormData(prev => ({...prev, propertySize: e.target.value}))} />
                    </div>
                    <div>
                      <Label htmlFor="rooms">Nombre de chambres</Label>
                      <Input id="rooms" type="number" value={formData.rooms} onChange={(e) => setFormData(prev => ({...prev, rooms: e.target.value}))} />
                    </div>
                    <div>
                      <Label htmlFor="livingRooms">Salons</Label>
                      <Input id="livingRooms" type="number" value={formData.livingRooms} onChange={(e) => setFormData(prev => ({...prev, livingRooms: e.target.value}))} />
                    </div>
                    <div>
                      <Label htmlFor="toilets">Toilettes</Label>
                      <Input id="toilets" type="number" value={formData.toilets} onChange={(e) => setFormData(prev => ({...prev, toilets: e.target.value}))} />
                    </div>
                  </div>
                </div>
              )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-2">
                      {vehicleSizes.map((size) => (
                        <div key={size} className="flex items-center space-x-2">
                          <Checkbox id={size} checked={formData.vehicleSizes.includes(size)} onCheckedChange={(checked) => handleArrayFieldChange('vehicleSizes', size, checked as boolean)} />
                  ...
                  <div>
                    ...
                    <div className="space-y-2 mt-2">
                      {carWashTypes.map((type) => (
                        <div key={type} className="flex items-center space-x-2">
                          <Checkbox id={type} checked={formData.carWashTypes.includes(type)} onCheckedChange={(checked) => handleArrayFieldChange('carWashTypes', type, checked as boolean)} />
                          <Label htmlFor={type} className="text-sm">{type}</Label>
                        </div>
                      ))}
                    </div>
                      ...
                      {degreasingItems.map((item) => (
                        <div key={item} className="flex items-center justify-between space-x-4">
                          <div className="flex items-center space-x-2">
                            <Checkbox id={item} checked={formData.degreasingItems.includes(item)} onCheckedChange={(checked) => handleArrayFieldChange('degreasingItems', item, checked as boolean)} />
                            <Label htmlFor={item} className="text-sm">{item}</Label>
                          </div>
                          {formData.degreasingItems.includes(item) && (
                            <Input type="number" placeholder="Quantité" className="w-20" value={formData.quantities[item] || ''} onChange={(e) => handleQuantityChange(item, parseInt(e.target.value) || 0)} />
                          )}
                        </div>
                      ))}
                    </div>
              ...
              {isServiceSelected('Nettoyage de piscine & Traitement de l\'eau') && (
                <div className="space-y-4 border-t pt-4">
                  ...
                  <div>
                    ...
                    <Input id="poolSize" type="number" value={formData.poolSize} onChange={(e) => setFormData(prev => ({...prev, poolSize: e.target.value}))} />
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4 border-t pt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {handymanServices.map((service) => (
                      <div key={service} className="flex items-center space-x-2">
                        <Checkbox id={service} checked={formData.handymanServices.includes(service)} onCheckedChange={(checked) => handleArrayFieldChange('handymanServices', service, checked as boolean)} />
              ...
              {isServiceSelected('Lutte antiparasitaire') && (
                <div className="space-y-4 border-t pt-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-2">
                      {pestTypes.map((pest) => (
                        <div key={pest} className="flex items-center space-x-2">
                          <Checkbox id={pest} checked={formData.pestTypes.includes(pest)} onCheckedChange={(checked) => handleArrayFieldChange('pestTypes', pest, checked as boolean)} />
                  ...
                  <div>
                    ...
                    <Input id="pestArea" type="number" value={formData.pestArea} onChange={(e) => setFormData(prev => ({...prev, pestArea: e.target.value}))} />
                  </div>
                </div>
              )}
              ...
              {isServiceSelected('Jardinage & Aménagement paysager') && (
                <div className="space-y-4 border-t pt-4">
                  ...
                  <div>
                    ...
                    <Input id="gardenArea" type="number" value={formData.gardenArea} onChange={(e) => setFormData(prev => ({...prev, gardenArea: e.target.value}))} />
                  </div>
                </div>
              )}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input id="photos" type="file" multiple accept="image/*" onChange={(e) => setFormData(prev => ({...prev, photos: e.target.files}))} />
                </div>
                <div>
                  <Label htmlFor="additionalNotes">Notes supplémentaires</Label>
                  <Textarea id="additionalNotes" value={formData.additionalNotes} onChange={(e) => setFormData(prev => ({...prev, additionalNotes: e.target.value}))} />
                </div>
              </div>
